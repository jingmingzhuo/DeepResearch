...
ALLOWED_TOOL_NAMES = ("search", "visit", "google_scholar")

SEARCH_VISIT_SYSTEM_PROMPT = """You are a deep research assistant. Your core function is to conduct thorough, multi-source investigations into any topic. You must handle both broad, open-domain inquiries and queries within specialized academic fields. For every request, synthesize information from credible, diverse sources to deliver a comprehensive, accurate, and objective response. When you have gathered sufficient information and are ready to provide the definitive response, you must enclose the entire final answer within <answer></answer> tags.
...
missing_tools = [tool for tool in ALLOWED_TOOL_NAMES if tool not in react_agent_module.TOOL_MAP]
if missing_tools:
    raise ValueError(f"Requested tools are not available: {missing_tools}")

react_agent_module.SYSTEM_PROMPT = SEARCH_VISIT_SYSTEM_PROMPT
react_agent_module.TOOL_MAP = {
    name: react_agent_module.TOOL_MAP[name] for name in ALLOWED_TOOL_NAMES
}
react_agent_module.TOOL_CLASS = [
    react_agent_module.TOOL_MAP[name] for name in ALLOWED_TOOL_NAMES
]
...
if __name__ == "__main__":
    ...
    model_dir = os.path.join(output_base, f"{model_name}_search_visit")
    ...
    print(f"Tools enabled: {', '.join(ALLOWED_TOOL_NAMES)}")

    data_filepath = f"eval_data/{args.dataset}"
    try:
    ...
    planning_ports = [
        int(port.strip())
        for port in os.getenv("MAIN_SERVER_PORTS", "6001").split(",")
        if port.strip()
    ]
    planning_rr_idx = 0
    question_to_ports = {}
